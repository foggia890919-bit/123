                ...
                {data.crossTab.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-2 text-right tabular-nums font-semibold text-green-700 bg-gray-50">{rowTotal > 0 ? rowTotal.toLocaleString("ko-KR") : "-"}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {data && data.lineItems.length > 0 ? (
          ...
          <div className="p-10 text-center text-gray-400 text-sm">
            {year}년 품목 데이터가 없습니다<br />
            <span className="text-xs">처방통계에 약품이 인식된 경우에만 표시됩니다</span>
          </div>
        )}
      </div>
    </div>
