...
const statusOptions = [
  { value: "PENDING",   label: "요청됨", cls: "bg-blue-50 text-blue-700 border-blue-200" },
  { value: "REVIEWING", label: "검토중", cls: "bg-yellow-50 text-yellow-700 border-yellow-200" },
  { value: "APPROVED",  label: "거래가능", cls: "bg-green-50 text-green-700 border-green-200" },
  { value: "REJECTED",  label: "거부됨", cls: "bg-red-50 text-red-700 border-red-200" },
];

export default function BizFilterRequestsPage() {
  return (
    <BizLayout>
      <FilterRequestsContent />
    </BizLayout>
  );
}

function FilterRequestsContent() {
  ...
  async function fetchReqs() {
    setLoading(true);
    try {
      const res = await fetch("/api/filter-request?scope=corp");
  ...
  async function updateStatus(id: string, status: string) {
    ...
    if (res.ok) {
      ...
      alert("권한이 없습니다. 본인이 매칭한 상위법인의 요청만 수정 가능합니다.");
    }
  }
  ...
  async function sendReply(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("권한이 없습니다. 본인이 매칭한 상위법인의 요청만 회신 가능합니다.");
      }
    } finally {
      setSavingId(null);
    }
  ...
  const statusCounts: Record<string, number> = useMemo(() => {
    ...
    for (const s of statusOptions) counts[s.value] = 0;
  ...
  const filtered = useMemo(() => reqs.filter((r) => {
    ...
    return (
      ...
      (r.upperCorpName || "").toLowerCase().includes(q) ||
      (r.lowerCorpName || "").toLowerCase().includes(q)
    );
  }), [reqs, statusFilter, query]);
  ...
  function exportExcel() {
    const rows = filtered.map((r) => ({
      ...
      상위법인: r.upperCorpName || "",
      하위법인: r.lowerCorpName || "",
    ...
    ws["!cols"] = [
      ...
      { wch: 16 }, { wch: 16 }, { wch: 18 }, { wch: 10 }, { wch: 40 }, { wch: 18 },
    ];
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-gray-900">필터링 요청 내역</h1>
        <p className="text-xs text-gray-500 mt-1">
          본인이 보낸 요청 + 매칭해둔 상위법인(UserClient · UPPER_CORP)이 받은 요청을 함께 표시합니다.
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-semibold text-gray-800">요청 목록 ({filtered.length}/{reqs.length}건)</h2>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="영업사원·거래처·사업자·제약사·법인 검색"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-xs text-gray-500 font-semibold">
                ...
                <th className="px-4 py-3 text-left">상위법인</th>
                <th className="px-4 py-3 text-left">하위법인</th>
              ...
              {pageRows.map((req) => {
                ...
                return (
                  <tr key={req.id} className="hover:bg-gray-50 align-top">
                    ...
                    <td className="px-4 py-3 text-gray-700 text-xs">{req.upperCorpName || <span className="text-gray-300">-</span>}</td>
                    <td className="px-4 py-3 text-gray-700 text-xs">{req.lowerCorpName || <span className="text-gray-300">-</span>}</td>
