...
export default function MedicationTable({ medications, loading, showCategoryB, showBioStatus, showOriginalDrug, showInsuranceCode, showNotes, showStock, showRate }: Props) {
  ...
  function searchSameIngredient(ingredientName: string) {
    window.location.href = `/search?q=${encodeURIComponent(ingredientName)}`;
  }
          ...
          {medications.map((med) => (
            <tr key={med.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 text-center">
                <button onClick={() => searchSameIngredient(med.ingredientName)}
              ...
              <td className="px-4 py-3 text-center">
                <button onClick={() => addToProposal(med)}
