...
export const OPTMAP_ARRAY_FORMULAS: Record<string, string> = {
  ...
  // J 적용원가 — 드롭다운이 있으면 개당원가 × 병수.
  //   단 품목명에 「+」가 있으면(조합) 그 자체가 옵션 전체이므로 ×1 (이중 곱 방지).
  //   드롭다운이 없으면 스크립트가 채운 자동원가(P) 를 보여준다.
  J2: `=ARRAYFORMULA(IF($I$2:$I="",$P$2:$P,IF(ISNUMBER($K$2:$K),$K$2:$K*IF(REGEXMATCH($I$2:$I&"","\\+"),1,IF($N$2:$N="",1,$N$2:$N)),"")))`,
...
export const OPTMAP_NOTES = [
  ...
  "★사장님 선택★ 이 옵션이 실제로 어떤 품목인지 목록에서 고르세요.\n· 단품(예: 피쿠알)을 고르면 옵션의 병수만큼 자동으로 곱해집니다 (3병 옵션 → ×3).\n· 1+1·교차처럼 묶음이면 품목사전에 그 조합(이름에 「+」 포함)을 등록해 고르세요.\n  조합은 그 자체가 옵션 전체라 병수를 곱하지 않습니다.",
  "🟢 실시간 — ★실제 적용되는 원가★\n= 개당원가 × 병수 (품목명에 「+」가 있는 조합은 ×1).\n품목을 고르거나 품목사전을 고치면 즉시 반영됩니다.\n안 골랐으면 시스템이 자동으로 읽은 원가가 표시됩니다.",
