...
이익이 자동으로 계산되게 하려면 **「⭐옵션매핑」 탭에 원가만 채우면** 됩니다. 코드나 설정 파일은 건드릴 필요 없습니다.

1. **「⭐옵션매핑」 탭**을 엽니다. 판매된 상품은 시스템이 **자동으로 줄을 만들어 둡니다** (직접 추가 안 하셔도 됩니다).
2. **「원가(개당)」** 칸에 그 상품 1개당 매입원가를 숫자만 넣습니다.
   - **「옵션관리번호」가 빈 줄 = 그 상품의 대표 줄**입니다. 여기에 넣으면 **그 상품 모든 옵션에 자동 적용**됩니다. 보통 이 줄 하나만 채우면 끝입니다.
   - 특정 옵션만 원가가 다르면, **그 옵션 줄에만** 따로 적으세요. 그 줄이 우선합니다.
   - 3병 세트처럼 여러 개가 묶인 옵션은 **개당 원가만 넣으면 병 수만큼 자동으로 곱해집니다.**
3. **「물류비(건당)」** 칸에 한 번 보낼 때 나가는 **실제 택배비**를 넣습니다. 묶음배송(한 주문에 여러 상품)이면 **1회만** 차감됩니다.
4. 나머지 칸(라벨·유형·메모)은 비워두셔도 됩니다.

> 각 칸 제목에 마우스를 올리면 설명이 뜹니다.
> 원가를 안 넣은 상품은 이익 칸이 **「설정 필요」**로 표시됩니다 (0원으로 잘못 계산하지 않습니다).
> 매일 아침 보고 때, 원가가 비어 있는 상품을 매출 큰 순으로 텔레그램으로 알려드립니다.
