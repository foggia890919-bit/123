...
## 모델 선택 (Flash → Pro 자동 폴백)

- **1차**: `gemini-2.5-flash` — 1~2초, 장당 수 원
- **2차(자동)**: `gemini-2.5-pro` — 1차 결과가 "빈손"(병원명 빈 문자열 **그리고**
  금액 0) 이면 자동 재시도. 3~5초, 장당 ~₩2.
- 운영자는 응답의 `debug.fallbackUsed: true` 로 어떤 사진이 Pro 까지 갔는지 추적 가능.
  운영 데이터로 폴백 비율이 높으면 1차를 Pro 로 승격 검토.
- 모든 호출은 `temperature: 0` (결정론적 출력 — 같은 사진 → 매번 같은 결과).

> **OCR 안 거침.** 사진 base64 를 Gemini 멀티모달 vision 에 그대로 넘긴다
> (`inlineData`). Clova OCR / Document AI 같은 사전 처리 없음 — 비정형 사진에서
> OCR 이 글자 형태로만 보고 헷갈리는 케이스를 LLM 이 문맥으로 추론해서 회수.
